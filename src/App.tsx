...
import { auth, loginWithGoogle, logout, hasConfig } from './lib/firebase';
import { onAuthStateChanged, User } from 'firebase/auth';
import { LogIn, Users, LogIn as LoginIcon, LogOut, Activity, MessageSquare, Phone, Home, Search as SearchIcon, Zap, Tag, UserCheck, UserX, Menu, X, ChevronRight, AlertTriangle } from 'lucide-react';
...
export default function App() {
  ...
  const [configMissing, setConfigMissing] = useState(!hasConfig);
      ...
      <div className="h-screen w-full flex items-center justify-center bg-[#1e293b] p-6 overflow-hidden relative">
        ...
        >
          <div className="w-24 h-24 bg-[#1e293b] rounded-[30px] flex items-center justify-center mx-auto mb-8 shadow-2xl transform rotate-3 hover:rotate-0 transition-transform duration-500">
             <span className="text-4xl font-black text-white tracking-tighter">{configMissing ? '!' : 'PS'}</span>
          </div>
          
          {configMissing ? (
            <>
              <h1 className="text-2xl font-black text-rose-500 mb-4 uppercase tracking-tighter leading-none italic">Configuration Missing</h1>
              <p className="text-slate-500 mb-8 text-sm font-medium leading-relaxed">
                আপনার Vercel বা Netlify Environment Variables কনফিগার করা হয়নি। দয়া করে ড্যাশবোর্ডে গিয়ে Firebase-এর এপিআই কীগুলো যুক্ত করুন।
              </p>
              <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 text-left mb-8">
                 <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-2 flex items-center gap-2">
                    <AlertTriangle size={12} /> Instructions for Vercel/Netlify:
                 </p>
                 <ul className="text-[9px] text-amber-700 font-bold space-y-1 list-disc pl-4">
                    <li>Go to Project Settings</li>
                    <li>Find "Environment Variables" section</li>
                    <li>Add 6 keys starting with VITE_FIREBASE_*</li>
