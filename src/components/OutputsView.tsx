...
export function OutputsView() {
  const [records, setRecords] = useState<ProductionOutput[]>([]);
  const [isAdding, setIsAdding] = useState(false);
  const [editingRecord, setEditingRecord] = useState<ProductionOutput | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ProductionOutput));
      setRecords(data);
      setLoading(false);
    });
    return unsubscribe;
  ...
  const handleSave = async (formData: any) => {
    try {
      if (editingRecord) {
        await updateDoc(doc(db, 'outputs', editingRecord.id!), formData);
      } else {
        await addDoc(collection(db, 'outputs'), formData);
      }
      setIsAdding(false);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-24">
      <div className="p-6">
        <h1 className="text-2xl font-bold text-[#1e293b] mb-6">Production Output</h1>
      ...
      <AnimatePresence>
        {isAdding && (
          <CommonRecordForm 
            title={editingRecord ? "Update Output" : "Add New Output"}
