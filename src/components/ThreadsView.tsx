...
export function ThreadsView() {
  ...
  const [editingRecord, setEditingRecord] = useState<ThreadRecord | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ThreadRecord));
      setRecords(data);
      setLoading(false);
    });
    return unsubscribe;
  ...
  const handleSave = async (formData: any) => {
    try {
      if (editingRecord) {
        await updateDoc(doc(db, 'threads', editingRecord.id!), formData);
      } else {
        await addDoc(collection(db, 'threads'), formData);
      }
      setIsAdding(false);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-24">
      <div className="p-6">
        <h1 className="text-2xl font-bold text-[#1e293b] mb-6">Thread Tracking</h1>
          ...
          <div className="divide-y divide-slate-50 min-h-[300px]">
             {loading ? (
                ...
                >
                  ...
                  <span className="text-slate-700 font-bold truncate">{record.tex_type}</span>
                  <span className="flex items-center gap-1 text-blue-600 font-bold">
                    <Clock size={10} /> {record.total_time || '0h'}
                  </span>
                  <span className="text-[#1e293b] font-black">{record.quantity || 0}</span>
                </div>
              ))}
          </div>
        </div>
      ...
      <AnimatePresence>
        {isAdding && (
          <CommonRecordForm 
            title={editingRecord ? "Update Thread Log" : "New Thread Log"}
